...
const Works = () => {
    const [projects, setProject] = useState([]); // variable to store the data from the API
    useEffect(() => {
      const query = '*[_type == "projects"]'; // query to fetch the data from the API
      client
        .fetch(query)
        .then((res) => setProject(res))
        .catch((err) => console.log(err)); // fetching the data from the API
    }, []);
    ...
    <div>
        ...
        {categorynav.map((cat, index) => {
          return (
            <span
              ...
              className={`${
                item === cat.category ? "bg-red-500" : " "
              } cursor-pointer border py-2 px-4 rounded-xl bg-blue-500 text-white font-medium`}
              key={index}
            >
              {cat.category}
            </span>
      ...
      <div className="work_container container mt-8 grid gap-8 sm:gap-12 md:gap-16 grid-cols-2 md:grid-cols-4">
        {projects.map((item) => {
          return (
            <div
              className="flex justify-center w-full col-span-1"
              key={item.id}
            >
